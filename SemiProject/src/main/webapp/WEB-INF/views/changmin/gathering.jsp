...
<%-- 					<%	if( paging.getCurPage() != 1) { %>
				...
				<% } else {%>
					...
					<%	for(int i=paging.getStartPage(); i<=paging.getEndPage(); i++) { %>
					<%		if( i == paging.getCurPage() ) { %>
					<li class="active"><a href="./gathering?curPage=<%=i %>&searchList=<%=searchList%>&word=<%=wordParam%>"><%=i %></a></li>
					<%		} else { %>
					<li><a href="./gathering?curPage=<%=i %>&searchList=<%=searchList%>&word=<%=wordParam%>"><%=i %></a></li>
					<%		} %>
					<%	} %>
			
					<%	if( paging.getCurPage() != paging.getTotalPage() ) { %>
					<li><a href="./gathering?curPage=<%=paging.getCurPage() + 1 %>&searchList=<%=searchList%>&word=<%=wordParam%>">&gt;</a></li>
					<%	} %>				
				<% } %>
