...
<div class="myContainer">
	...
	<div class="right">
		<h1>소모임게시판</h1>
			<form name="frm" method="GET" action="./gathering">
...
<%-- 					<%	if( paging.getCurPage() != 1) { %>
				...
				<% if( wordParam ==null ) { %>
					...
					<li class="active"><a href="./gathering?curPage=<%=i %>"><%=i %></a></li>
					<%		} else { %>
					<li><a href="./gathering?curPage=<%=i %>"><%=i %></a></li>
					<%		} %>
					<%	} %>
				...
				<% } else {%>
					<%	if( paging.getCurPage() != 1) { %>
					<li><a href="./gathering?curPage=<%=paging.getCurPage() - 1 %>&word=<%=wordParam%>">&lt;</a></li>
					<%	} %>
					
					<%	for(int i=paging.getStartPage(); i<=paging.getEndPage(); i++) { %>
					<%		if( i == paging.getCurPage() ) { %>
					<li class="active"><a href="./gathering?curPage=<%=i %>&searchList=<%=request.getParameter("searchList")%>&word=<%=wordParam%>"><%=i %></a></li>
					<%		} else { %>
					<li><a href="./gathering?curPage=<%=i %>&searchList=<%=request.getParameter("searchList")%>&word=<%=wordParam%>"><%=i %></a></li>
					<%		} %>
					<%	} %>
			
					<%	if( paging.getCurPage() != paging.getTotalPage() ) { %>
					<li><a href="./gathering?curPage=<%=paging.getCurPage() + 1 %>&searchList=<%=request.getParameter("searchList")%>&word=<%=wordParam%>">&gt;</a></li>
					<%	} %>				
				<% } %>
